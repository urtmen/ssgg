...
const descSets = {
  ...
  'Бронежилет Отмены' : {
    ...
    desc:     'После прохождения игры понижает фильтр GG в колесе игр на 25 (и так 3 раза по 1 на игру). Бронежилет не может быть применен к стримеру, если в инвентаре уже есть Бронежилет из дропсов, так как нельзя надеть два бронежилета одновременно. При этом прочность Бронежилета из дропсов не уменьшается. Остальные предметы, отражающие дебаффы, работают по их собственным правилам. Не может выпасть больше одного за раз.'
  },
  'Шпаргалка' : {
    buffs:       ' ',
    perm:    ' ',
